...
void loop(){
  ...
  if(manual){
    ...
    if(getKlasse() == ACK_MOTION || getKlasse() == MOTION){
      s_seatsVal = (bool)getWert();
      s_seats();
    }
...
void s_temperature(){
	...
	thermoMapVal = map(thermoVal, 15, 35, 0, 19);
...
void s_seats(){
  int i = 0;
  if(!s_seatsVal){
    for(i = seats_start; i < seats_end; i++){
      strip.setPixelColor(i, 0, brightness, 0);
    }
  }else{
    for(i = seats_start; i < seats_end; i++){
      strip.setPixelColor(i, 0, brightness, 0);
      strip.setPixelColor(seats_start, brightness, 0, 0);
      strip.setPixelColor(seats_start + 1, brightness, 0, 0);
      strip.setPixelColor(seats_start + 2, brightness, 0, 0);
      strip.setPixelColor(seats_start + 3, brightness, 0, 0);
    }
  }
}

void s_soundbars(){
  soundMapVal = map(soundVal, 300, 800, 0, 127);
  
  //calculate which leds in each column have to shine with the intensity depending on the random value
  for(i = 0; i <= spalten; i++){        
    strip.setPixelColor(sound[i][0] , max(0, soundMapVal - 110), max(0, soundMapVal - 110), max(0, soundMapVal - 110));
    ...
    strip.setPixelColor(sound[i][6] , max(0, soundMapVal - 110), max(0, soundMapVal - 110), max(0, soundMapVal - 110));
  }
}
